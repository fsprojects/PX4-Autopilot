...
**Status**: 🔄 ACTIVE — 388 theorems · 187 verified examples · 6 `sorry` · Lean 4.29.0

## Last Updated

- **Date**: 2026-04-26 01:00 UTC
- **Commit**: `424ec0b562`
...
The Lean Squad has formally verified **388 named theorems and 187 verified examples** across
**29 Lean 4 files**, covering the core mathematical utility library (`src/lib/mathlib/`),
...
`ObstacleMath::wrap_bin` + `get_bin_at_angle` + `get_lower_bound_angle` + `get_offset_bin_index`
(`src/lib/collision_prevention/`), the CCITT CRC-16 signature (`src/lib/crc/crc.c`),
piecewise-linear sqrt (`SqrtLinear.lean`), and braking-distance safety
...
and 334 correspondence tests). **Six** `sorry`-guarded theorems remain — all six in
`WrapAngle.lean` pending Mathlib floor arithmetic. All other 28 targets are sorry-free,
verified by `lake build` with Lean 4.29.0. Runs 67–71 added `GetBinAtAngle` (13 theorems),
`GetLowerBoundAngle` (13 theorems), `CollisionPrevComposition` (8 cross-module composition
theorems), `BrakingDist` (14 theorems), and `Crc16Sig` (8 theorems for the CCITT firmware
CRC), bringing total from 349 to 388 theorems.
...
graph TD
    ...
    L2a["Layer 2: Signal Filters<br/>AlphaFilter · SlewRate · Deadzone · MedianFilter<br/>45 theorems · 26 examples"]
    L2b["Layer 3: Basic Curves<br/>Interpolate · Lerp · Expo · SuperExpo<br/>54 theorems · 21 examples"]
    ...
    L8["Layer 8: Physical Models<br/>Atmosphere (18 thms) · SqrtLinear (15 thms)<br/>SignFromBoolSq · BrakingDist (14 thms)<br/>52 theorems · 12 examples"]
    L9["Layer 9: Collision Prevention<br/>GetBinAtAngle · GetLowerBoundAngle<br/>CollisionPrevComposition<br/>34 theorems · 12 examples"]
    L1 --> L2a
    L1 --> L2b
    L2b --> L2c
    L1 --> L4
...
### Layer 7 — Protocol Utilities (2 files, 16 theorems, 12 examples)
...
integrity verification and bootloader packet checksums. `CRC16_OUTPUT_XOR = 0x0000`
so `crc16_signature` reduces to a pure fold of `crc16_add`, enabling the same
fold/split theorem structure.

```mermaid
graph LR
    CF["Crc16Fold.lean<br/>8 theorems · 6 examples<br/>Septentrio CCITT CRC-16"]
    CS["Crc16Sig.lean<br/>8 theorems · 6 examples<br/>Firmware CCITT CRC-16"]
    S1["crc16Step<br/>Septentrio per-byte update"]
    S2["crc16Add<br/>Bit-by-bit per-byte update"]
```

**Key results (both files)**:
- `crc16_append` / `crc16sig_append` (**fold/split**): proves streaming CRC correctness.
- `crc16_nil` / `crc16sig_nil`: empty buffer yields CRC = initial value.
- `crc16_append3` / `crc16sig_append3`: three-part split for chunked packet processing.
- Concrete `native_decide` examples validate polynomial correctness.

**Correspondence**: **exact** — `UInt8`/`UInt16` modular arithmetic matches C exactly.
...
| `SignFromBoolSq.lean` | 17 | 5 | 0 | ✅ Phase 5 | `signFromBool` (range ±1, ne_zero) + `sq` (non-neg, even, iff-zero, mul) |
| `Crc16Fold.lean` | 8 | 6 | 0 | ✅ Phase 5 | CRC-16 fold/split: streaming correctness, CCITT polynomial validated |
| `Crc16Sig.lean` | 8 | 6 | 0 | ✅ Phase 5 | CRC-16 add/signature (bit-by-bit CCITT): fold/append, init-value generality |
| `Atmosphere.lean` | 18 | 12 | 0 | ✅ Phase 5 | ISA atmosphere: temp/density/pressure; 3 monotonicity theorems proved |
| `SqrtLinear.lean` | 15 | 0 | 0 | ✅ Phase 5 | Piecewise sqrt: neg/identity proved; sqrt branch via explicit axioms |
| `WrapBin.lean` | 20 | 9 | 0 | ✅ Phase 5 | Euclidean-mod wrap + C++ bug confirmed — 🐛 latent bug |
| `BrakingDist.lean` | 14 | 7 | 0 | ✅ Phase 5 | Braking distance: non-neg, mono, quadratic scaling, no-delay formula |
| `GetBinAtAngle.lean` | 13 | 8 | 0 | ✅ Phase 5 | Angle-to-bin: range, periodicity, offset rotation |
| `GetLowerBoundAngle.lean` | 13 | 12 | 0 | ✅ Phase 5 | Lower-bound angle: range invariant, periodicity, offset |
| `CollisionPrevComposition.lean` | 8 | 4 | 0 | ✅ Phase 5 | Rotation group + lower-bound commutativity |
| `Basic.lean` | — | — | — | ✅ | Barrel file |
| **Total** | **388** | **187** | **6** | — | **3 bugs found; 6 sorry in WrapAngle (floor arithmetic)** |
...
> *`lake build` verified with Lean 4.29.0. 6 `sorry` remain (all in WrapAngle wrapRat — need Mathlib floor).*
> *388 theorems (382 proved) across 29 files. 9 proof layers. 3 bugs found.*
