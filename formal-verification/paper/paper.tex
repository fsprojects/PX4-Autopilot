...
\title{Formal Verification of a UAV Flight Controller's Core Libraries with Lean~4:\\
       259 Theorems, Two Bugs, 6 Sorry in 20 Modules}
...
(no Mathlib), we verified 253 theorems fully (6~\texttt{sorry}-guarded) across
20~Lean modules covering 24~C++ targets including signal-processing filters,
rate-limiting, piecewise-linear interpolation, online statistics, ring buffers,
a time-delayed boolean state machine, fundamental arithmetic utilities, and a
protocol CRC-16 streaming correctness property. All proofs were mechanically
checked by \texttt{lake build}. The project discovered two genuine implementation
bugs through failed proof attempts: (1)~\texttt{signNoZero<float>} silently
returns~0 for NaN inputs, violating a non-zero safety contract, and
(2)~\texttt{negate<int16\_t>} is not a global involution due to an incorrect
\texttt{INT16\_MAX} special case. Six theorems in one file remain
\texttt{sorry}-guarded, pending floor-arithmetic support from Mathlib.
We describe the modelling strategy, proof tactics, correspondence discipline,
and lessons learned in applying interactive theorem proving to a production
embedded codebase without network access to the Mathlib cache.
\end{abstract}
...
\begin{itemize}
  \item A formal verification campaign covering \textbf{24~C++ targets with Lean proofs}
        in 20~Lean modules, yielding \textbf{253~fully proved theorems} (259~stated;
        6~\texttt{sorry}-guarded in one file) plus 129~concrete examples.
  ...
  \item A \textbf{correspondence discipline} documenting modelling approximations and
        their impact on proof validity for all 20 Lean files.
...
Table~\ref{tab:theorems} summarises the 20~Lean files (24~C++ targets). In total,
\textbf{259~theorem statements} were made, of which \textbf{253~are fully proved}
and \textbf{6~remain \texttt{sorry}-guarded} (all in \texttt{WrapAngle.lean},
requiring Mathlib's floor-division lemmas). Additionally, \textbf{129~concrete
\texttt{example}/\texttt{\#eval} checks} serve as property-based regression tests.

\begin{table}[t]
\centering
\caption{Lean 4 verification summary by file (20 files, 24 C++ targets).
...
\texttt{WrapAngle}     & \texttt{wrap\_pi} (int+rat)        & 15 & 6 & Range, idempotent, period 🔄 \\
...
\texttt{InterpolateN}  & \texttt{interpolateN} (N=2,3)      & 19 & 0 & Node exact, range, mono ✅ \\
...
\texttt{Crc16Fold}     & \texttt{buffer\_crc16}             &  8 & 0 & Fold/split, streaming CRC ✅ \\
\hline
\textbf{Total}         &                                    & \textbf{259} & \textbf{6} & \\
\bottomrule
\end{tabular}
\end{table}
...
libraries using Lean~4. Across 20~Lean modules and 24~C++ targets, we proved
253~theorems covering rate limiting, interpolation, filtering, statistics, data
structures, a time-delayed state machine, and a protocol CRC streaming property.
The campaign discovered two genuine implementation bugs, both verified by
mechanically-checked counterexamples. The project demonstrates that interactive
theorem proving can be usefully applied to production embedded C++ at the library
level, even under significant toolchain constraints.

Future work includes: resolving the 6~remaining sorrys in \texttt{WrapAngle.lean}
once Mathlib becomes available; verifying the atmospheric density model
(\texttt{src/lib/atmosphere/}); and formalising the Commander arming finite-state
machine (\texttt{src/modules/commander/}) — the most complex remaining target.
