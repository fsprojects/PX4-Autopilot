...
\title{Formal Verification of a UAV Flight Controller's Core Libraries with Lean~4:\\
       389 Theorems, Three Bugs, Zero Sorry in 33 Modules}
...
(no Mathlib), we verified \textbf{389~theorems} fully across \textbf{33~Lean modules}
covering \textbf{37~C++ targets} including signal-processing filters, rate-limiting,
...
streaming correctness, and integer range utilities. All proofs are mechanically
...
The project discovered \textbf{three genuine implementation bugs} through proof
...
\begin{itemize}
  \item A formal verification campaign covering \textbf{37~C++ targets with Lean proofs}
        in 33~Lean modules, yielding \textbf{389~fully proved theorems} with
  ...
  \item Discovery of \textbf{three genuine implementation bugs} through proof failure
  ...
  \item A \textbf{correspondence discipline} documenting modelling approximations and
        their impact on proof validity for all 33 Lean files, including 4~327~executable
...
Table~\ref{tab:theorems} summarises the 33~Lean files (37~C++ targets). In total,
\textbf{389~theorems} are stated and proved with zero \texttt{sorry} in any proof
...
\begin{table}[t]
\centering
\caption{Lean 4 verification summary by file (33 files, 37 C++ targets).
...
\midrule
\texttt{MathFunctions} & \texttt{constrain}, \texttt{signNoZero}, \texttt{countSetBits} & 16 & Range clamp, sign, bits ✅ \\
\texttt{SlewRate}      & \texttt{SlewRate::update}          & 18  & Rate limit, no-overshoot, conv. ✅ \\
\texttt{Deadzone}      & \texttt{deadzone}                  & 13 & Zero-zone, odd symmetry ✅ \\
\texttt{AlphaFilter}   & \texttt{AlphaFilter::updateCalc.}  & 13 & No-overshoot, closed form ✅ \\
\texttt{Interpolate}   & \texttt{interpolate}               & 10 & Monotone, range ✅ \\
\texttt{WelfordMean}   & \texttt{WelfordMean::update}       & 13 & Mean invariant, M2~$\geq0$ ✅ \\
\texttt{WrapAngle}     & \texttt{wrap\_pi} (int+rat)        & 16 & Range, idempotent, period ⚠ \\
\texttt{Lerp}          & \texttt{lerp}                      & 14 & Endpoints, range, monotone ✅ \\
\texttt{Negate}        & \texttt{negate<int16\_t>}          & 13 & 🐛 Not involution ✅ \\
\texttt{Expo}          & \texttt{expo}                      & 17 & Odd, range, fixed pts ✅ \\
\texttt{RingBuffer}    & \texttt{TimestampedRingBuffer}     & 44 & FIFO, cap, index, wrap ✅ \\
\texttt{MedianFilter}  & \texttt{MedianFilter}              & 10 & Sort order, range ✅ \\
\texttt{SuperExpo}     & \texttt{superexpo}                 & 19 & Odd, denom~$>0$, range ✅ \\
...
\texttt{InterpolateN}  & \texttt{interpolateN} (N=2,3)      & 21 & Node exact, range, mono ✅ \\
...
\texttt{Crc64}         & \texttt{crc64\_add\_word}          &  8 & CRC-64-WE fold/split, chunk ✅ \\
\texttt{IsInRange}     & \texttt{math::isInRange}           & 13 & Closed interval, mono, shift ✅ \\
\texttt{ConstrainToInt16} & \texttt{constrainFloatToInt16} & 13 & Range, clamp, id, mono, idem ✅ \\
\texttt{CommanderArming}& \texttt{Arming FSM}               & 20 & State inv, transitions ✅ \\
\texttt{Atmosphere}    & density, temperature               & 20 & Formula fidelity, monotone ✅ \\
\texttt{WrapBin}       & \texttt{wrap\_bin}                 & 27 & 🐛 Negative index bug ✅ \\
\texttt{SqrtLinear}    & \texttt{sqrtLinear}                & 19 & Range, id, monotone ⚠ \\
\texttt{BrakingDist}   & \texttt{BrakingDistance}           & 16 & Pos, mono, closed form ✅ \\
\texttt{GetBinAtAngle} & \texttt{getBinAtAngle}             & 14 & Range, exact, comp ✅ \\
\texttt{GetLowerBound} & \texttt{getLowerBoundAngle}        & 14 & Round-trip, range ✅ \\
\texttt{CollisionPrev} & collision pipeline                 & 14 & Safety, composition ✅ \\
\texttt{Other}         & \texttt{Crc16Fold, Basic, ...}     & ---& Infrastructure \\
\hline
\textbf{Total}         &                                    & \textbf{389} & \textbf{0 sorry, 16 axioms} \\
\bottomrule
\end{tabular}
\end{table}
...
Across 33~files, roughly 50\% of theorems were proved by \texttt{decide} or
...
\begin{itemize}
  ...
  \item \textbf{Proof failures are findings.} All three bugs were discovered through
        ...
        (negate), or a correspondence test exposed the Lean/C++ divergence (wrap\_bin).
...
libraries using Lean~4. Across 33~Lean modules and 37~C++ targets, we proved
389~theorems covering rate limiting, interpolation, filtering, statistics, data
structures, a time-delayed state machine, CRC streaming properties, and integer
range containment. The campaign discovered three genuine implementation bugs, all
